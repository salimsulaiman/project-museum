...
@section('content')
    <div>
        ...
        <button type="button" class="btn btn-success" data-bs-toggle="modal"
            data-bs-target="#updateService{{ $profileSection->id }}">Ubah
            Section</button>
        <div class="modal fade" id="updateService{{ $profileSection->id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
            aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <form method="POST" action="{{ route('admin.profile-section.update') }}" enctype="multipart/form-data">
                        @csrf
                        @method('put')
                        <div class="modal-body px-4">
                            <input type="hidden" name="id" value="{{ $profileSection->id }}">

                            <div class="mb-3">
                                <label for="content" class="form-label">Konten</label>
                                <div class="quill-editor" style="height: 250px;" data-target="content-update">
                                    {!! old('content', $profileSection->content) !!}
                                </div>
                                <input type="hidden" name="content" id="content-update"
                                    value="{{ old('content', $profileSection->content) }}">
                            </div>
